...
from verifierlab.campaigns.engine import freeze_run, init_workspace, release_labels, run_campaign
...
def test_refund_ci_smoke(tmp_path: Path) -> None:
    ...
    # Recover planted classes via taxonomy on exploit records (≥3 target for §38).
    classes = set()
    ...
    assert (result.manifest.metadata or {}).get("gt_evaluated_on") == "coordinator"
...
def test_freeze_and_label_release(tmp_path: Path) -> None:
    ...
    vault = LabelVault(result.run_dir / "vault")
